...
export const vehicles = pgTable(
  ...
  (table) => ({
    searchVectorIdx: index("vehicles_search_vector_idx").using(
      "gin",
      sql`(
        setweight(to_tsvector('simple', COALESCE(${table.licensePlate}, '')), 'A') ||
        setweight(to_tsvector('simple', COALESCE(${table.driverName}, '')), 'B') ||
        setweight(to_tsvector('simple', COALESCE(${table.driverPhone}, '')), 'C')
      )`
    ),
  })
);