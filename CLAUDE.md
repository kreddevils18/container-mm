...
## 🏗️ **SECTION 1: ARCHITECTURE BLUEPRINT**

_Foundation and design principles_
...
_How to write and organize code_
...
## 📚 **SECTION 3: BEST PRACTICES MANUAL**

_Patterns and recommended approaches_
...
# Development Tools
pnpm install -D @testing-library/react @testing-library/jest-dom vitest jsdom
```
...
_Testing, validation, and quality gates_
...
- [ ] ESLint passes with ZERO warnings (`pnnpm run lint`)
...
_Optimization and security requirements_
...
Environment validation schemas, security best practices, and performance monitoring are covered in the Security Requirements and Performance Guidelines sections above.

---

## 📋 **QUICK REFERENCE**

_Summary of key requirements_

### File Limits

- **Max 500 lines per file** - Split if larger
- **Max 200 lines per component** - Refactor if larger
- **Max 50 lines per function** - Keep functions focused

### Type Safety

- **Use `ReactElement` not `JSX.Element`** - Modern React 19 typing
- **Never use `any` type** - Use proper typing or `unknown`
- **Explicit return types required** - For all functions and components

### Validation

- **Zod for ALL external data** - API responses, forms, environment
- **Branded types for IDs** - Type-safe domain-specific values
- **Environment validation required** - Parse with Zod schemas

### Testing

- **80% minimum coverage** - NO EXCEPTIONS
- **Co-located in `__tests__/`** - Tests next to components
- **Test user behavior** - Not implementation details

### Quality Gates

- **TypeScript: Zero errors** - `pnpm run type-check` must pass
- **ESLint: Zero warnings** - `pnpm run lint` must pass
- **Tests: 80%+ coverage** - `pnpm run test:coverage` must pass
- **Documentation: Complete JSDoc** - ALL exports documented
